...
export default async function UserDetailPage({ params }: Props) {
  ...
  const stats: { label: string; value: string | number }[] = [
    ...
    { label: "받은 평점", value: user._count.ratingsReceived },
    ...
    <div className="space-y-4 max-w-3xl">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {user.role === "SUPER_ADMIN" ? (
                ...
                <Badge variant="secondary">
                  업체 어드민
                  {user.managedBusiness ? ` · ${user.managedBusiness.name}` : ""}
                </Badge>
              ) : (
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <div className="space-y-4">
          <NicknameEditor userId={user.id} initialNickname={user.nickname} />
          ...
          <Card>
            ...
            <CardContent className="p-4 sm:p-6 pt-0">
              ...
              <Row
                label="흡연 / 음주"
                value={[p?.smokingStatus, p?.drinkingStatus].filter(Boolean).join(" / ") || null}
              />
              <Row label="결혼 여부" value={p?.maritalStatus} />
              <Row label="자기소개" value={p?.bio} />
              {user.isBlocked && <Row label="정지 사유" value={user.blockReason} />}
              {user.managedBusiness && (
                <Row
                  label="연결 업체"
                  value={user.managedBusiness.name}
                />
              )}
            </CardContent>
          </Card>
            ...
            <CardContent className="p-4 sm:p-6 pt-0">
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {stats.map((s) => (
                  <div
                    key={s.label}
                    className="rounded-lg border bg-primary/5 border-primary/10 p-3 text-center"
                  >
